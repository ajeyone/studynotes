...
case $var in
  pattern1)
  	command1 xx x xx x x
  	command2 xxx x x x x
  ;;
  pattern2|pattern3)
  	command3 xx x x xx
  ;;
...
这是一个神奇的语法，目的是计算子字符串，思路是“匹配”+“删除”，使用字符或通配符以及匹配规则来匹配内容，然后删除掉这个内容，来达到计算子字符串的目的。

匹配规则分为 4 种，如下表所示：

| 规则         | 符号 | 语法                      | 含义                                   |
| ------------ | ---- | ------------------------- | -------------------------------------- |
| 左侧最小匹配 | #    | ${string#delete_pattern}  | 从头开始匹配，找到最短的即匹配成功。   |
| 左侧最大匹配 | ##   | ${string##delete_pattern} | 从头开始匹配，找到最长的才匹配成功。   |
| 右侧最小匹配 | %    | ${string%delete_pattern}  | 从末尾开始匹配，找到最短的即匹配成功。 |
| 右侧最大匹配 | %%   | ${string%%delete_pattern} | 从末尾开始匹配，找到最长的才匹配成功。 |

示例

```sh
filename=/Users/me/12345.txt

# 获取文件的扩展名
echo ${filename##*.} # 左侧匹配，最大匹配，即删除最后一个"."以及前面的所有字符。
# output: txt

# 去掉文件的扩展名
echo ${filename%.*} # 右侧匹配，最小匹配，即删除最后一个"."以及后面的所有字符。
# output: /Users/me/12345
```

该方法只能“掐头去尾”，要么去掉开头的，要么去掉末尾的，一些复杂的操作需要使用多次才行，例如

```sh
# 将一个文件移动到它的爷爷目录下，或者说与它的父目录平行。
filename=/Users/me/12345.txt
temp1=${filename%/*}     # 去掉末尾的 /12345.txt，得到 /Users/me
temp2=${temp1%/*}        # 再次去掉末尾的 /me，得到 /Users
temp3=${filename##*/}    # 去掉前面的所有目录，得到 12345.txt
target=${temp2}/${temp3} # 拼接目录和文件，得到 /Users/12345.txt
mv "$filename" "$target" # 移动
```
